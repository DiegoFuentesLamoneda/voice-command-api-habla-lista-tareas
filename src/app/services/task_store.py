...
Cada tarea es un diccionario con las claves `id` (int), `title` (str) y
`done` (bool). Las funciones devuelven copias para que nadie mute el almacen
por accidente desde fuera.
"""
...
def create_task(title: str, done: bool = False) -> dict:
    ...
    with _lock:
        task = {"id": next(_id_sequence), "title": title, "done": done}
        tasks.append(task)
        return task.copy()
...
def replace_task(task_id: int, title: str, done: bool) -> dict | None:
